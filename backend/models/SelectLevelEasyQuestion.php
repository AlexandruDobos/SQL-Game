...
$stmt = $conn->prepare("SELECT max(id) AS max_id, count(*) AS nr_questions FROM `easy_queries`");
$stmt->execute();
...
if ($nr_questions > 10) {
    ...
    while (count($used_id) < 10) {
        ...
        while (!$semaphore) {
            ...
            $stmt = $conn->prepare("SELECT id FROM `easy_queries` WHERE id = '$question_id'");
            $stmt->execute();
        ...
        $stmt = $conn->prepare("SELECT * FROM `easy_queries` WHERE id = '$question_id'");
        $stmt->execute();
